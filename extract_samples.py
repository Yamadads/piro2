...
for case_name in ["bark", "bikes", "boat", "graf", "leuven", "trees", "wall"]:
    ...
    for row, col in feature.corner_peaks(feature.corner_harris(color.rgb2gray(images[0]))):
        if row < SPAN or images[0].shape[0] - row <= SPAN or col < SPAN or images[0].shape[1] - col <= SPAN:
            continue
        good = True
